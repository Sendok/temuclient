...
export async function enforceFeatureRateLimit(request: Request, scope: string, limit: number, windowSeconds: number) {
  ...
  const count = await withRedis(async () => { if (redis.status === "wait") await redis.connect(); const next = await redis.incr(key); if (next === 1) await redis.expire(key, windowSeconds); return next; }, 0, { operation: `rate_limit:${scope}` });
...
export async function enforceAIRateLimit(input: {
  ...
  const count = await withRedis(async () => { if (redis.status === "wait") await redis.connect(); const next = await redis.incr(key); if (next === 1) await redis.expire(key, windowSeconds); return next; }, 0, { operation: "rate_limit:ai" });
