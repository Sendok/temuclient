...
function AdminShell({ actorName, role, children }: { actorName: string; role: PlatformRole | null; children: React.ReactNode }) {
  ...
  const nav = <><div className="flex h-[72px] items-center border-b border-white/10 px-4"><TemuClientLogo inverse /><span className="ml-auto rounded-full border border-white/15 px-2 py-1 text-[9px] font-bold tracking-[0.12em] text-white/55">OPS</span></div><nav className="flex-1 space-y-1 overflow-y-auto p-3">{navigation.map(([label, href, Icon]) => { const active = pathname === href || (href !== "/admin" && pathname.startsWith(href)); return <Link className={cn("relative flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-medium transition", active ? "bg-white/10 text-white before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:rounded-full before:bg-[#ff8bb2]" : "text-white/55 hover:bg-white/[0.06] hover:text-white")} href={href} key={href} onClick={() => setOpen(false)}><Icon className={cn("size-4", active && "text-[#ff8bb2]")} />{label}</Link>; })}</nav><div className="border-t border-white/10 p-4"><p className="truncate text-sm font-semibold text-white">{actorName}</p><p className="mt-1 text-xs text-[#ff8bb2]">{role}</p><button className="mt-3 flex min-h-10 w-full items-center gap-2 rounded-lg text-sm text-white/55 transition hover:text-white" onClick={logout}><LogOut className="size-4" />Keluar</button></div></>;
  return <div className="min-h-screen bg-background"><aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-white/10 bg-[#071518] lg:flex">{nav}</aside><div className="lg:pl-64"><header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b bg-white/95 px-4 backdrop-blur sm:px-6"><button aria-label="Buka navigasi admin" className="flex size-10 items-center justify-center rounded-full border lg:hidden" onClick={() => setOpen(true)}><Menu className="size-4" /></button><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-brand-600">Trust & moderation</p><p className="mt-1 text-sm font-semibold">Admin Console</p></div><Badge tone="brand">{role}</Badge></header><main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8">{children}</main></div>{open && <div className="fixed inset-0 z-50 bg-[#071518]/40 lg:hidden" onClick={() => setOpen(false)}><aside className="absolute inset-y-0 left-0 flex w-[85%] max-w-xs flex-col bg-[#071518] text-white" onClick={(event) => event.stopPropagation()}><button aria-label="Tutup navigasi" className="absolute right-3 top-4 flex size-10 items-center justify-center text-white" onClick={() => setOpen(false)}><X className="size-5" /></button>{nav}</aside></div>}</div>;
}
